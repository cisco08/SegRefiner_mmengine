...
class SegRefinerSemantic(SegRefiner):
    ...
    def simple_test_semantic(self, img_metas, img, coarse_masks, **kwargs):
        ...
        ori_size_mask = F.interpolate(model_size_mask, size=tuple(ori_shape))
    ...
    def _get_global_input(self, img, coarse_masks, ori_shape, current_device):
        ...
        model_size = to_2tuple(model_size)
        coarse_mask = coarse_masks[0].masks[0]
        global_img = F.interpolate(img, size=model_size)
        global_mask = torch.tensor(coarse_mask, dtype=torch.float32, device=current_device)
        global_mask = F.interpolate(global_mask.unsqueeze(0).unsqueeze(0), size=model_size)
    ...
    def get_local_input(self, img, ori_size_mask, fine_probs, ori_shape):
        ...
        ori_size_fine_probs = F.interpolate(fine_probs, tuple(ori_shape))
        ...
        scale_factor_y, scale_factor_x = img_h / model_size[0], img_w / model_size[1]
