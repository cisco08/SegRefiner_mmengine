    ...
    def _get_object_input(self, img, coarse_masks, img_metas, current_device):
        ...
        for mask in coarse_masks:
            ...
            object_mask = torch.tensor(mask[y_1_ob: y_2_ob, x_1_ob: x_2_ob], 
                                        ...
                                        dtype=torch.float32)
            object_imgs.append(F.interpolate(object_img, size=model_size, mode='bilinear'))
            object_masks.append(F.interpolate(object_mask.unsqueeze(0).unsqueeze(0), size=model_size, mode='bilinear'))
