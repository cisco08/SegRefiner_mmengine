...
class SegRefiner(BaseModule):
    ...
    def forward_dummy(self, img, return_loss=True, **kwargs):
        ...
        if self.task == 'instance':
            ...
            masks = BitmapMasks.random(1,dtype=np.uint8, height=height, width=width)
            gt_bbox = np.array([[[0,0,height, width, 0.8, 1]]])
            dummy_img_metas[0]["ori_filename"] = 'test_hr.jpg'
            dummy_img_metas[0]['ori_shape'] = np.array([height, width,3])
            return self.simple_test_instance(dummy_img_metas, img, [masks], gt_bbox)
        elif self.task == 'semantic':
            from mmdet.core.mask.structures import BitmapMasks
            masks = BitmapMasks.random(1,dtype=np.uint8, height=height, width=width)
            dummy_img_metas[0]["ori_filename"] = 'test_hr.jpg'
            dummy_img_metas[0]['ori_shape'] = np.array([height, width,3])
            return self.simple_test_semantic(dummy_img_metas, img, [masks])
        else:
            raise ValueError(f'unsupported task type: {self.task}')
