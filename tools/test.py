...
def parse_args():
    ...
    parser.add_argument('--out_dir', help='direction for output result files')
...
def main():
    ...
    testsets = None
    if hasattr(dataset, 'testsets'):
        testsets = dataset.testsets
    if not distributed:
        model = build_dp(model, cfg.device, device_ids=cfg.gpu_ids)
        outputs = single_gpu_test(model, data_loader, task, testsets, out_dir=args.out_dir)
    else:
        model = build_ddp(
            model,
            cfg.device,
        ...
        outputs = multi_gpu_test(
            model, data_loader, task, testsets, args.out_dir,
            args.tmpdir, args.gpu_collect)

    if task == 'instance':
        rank, _ = get_dist_info()
        if rank == 0:
            dataset.format_results(outputs, args.out_prefix)
